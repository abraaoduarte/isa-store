...
import { LedgerFormValues, FormLedgerTemplateProps } from './Ledger.interface';
...
export const FormLedger: FC<FormLedgerTemplateProps> = ({
  pageTitle,
  ledgerId,
  categories,
}) => {
  ...
  >((data) => {
    ...
    return api.post('/ledger', normalizeData);
  });
  ...
  >((data) => {
    ...
    return api.patch(`/ledger/${ledgerId}`, normalizeData);
  });

  useEffect(() => {
    if (ledgerId && isSuccess) {
  ...
  }, [ledgerId, data, reset, isSuccess]);

  const onSubmit = (data: LedgerFormValues) => {
    ledgerId
      ? updateLedger.mutate(data, {
          onSuccess: () => {
            Router.push('/ledger');
          },
          onError: (error) => {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        >
          <LoadingButton
            ...
            variant="contained"
            disabled={
              !isEmpty(errors) || addLedger.isLoading || updateLedger.isLoading
            }
          >
            {ledgerId ? 'Atualizar' : 'Salvar'}
          </LoadingButton>
        </Box>
      </form>
    </>
